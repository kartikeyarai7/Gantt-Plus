...
const bpsCalModel = require('./bpscalendarmodel');
const bdsCalModel = require('./bdscalendarmodel');
...
async function main() {
  const uri = 'mongodb+srv://Dbadmin:Bareilly7@cluster0.ap6t6.mongodb.net/myFirstDatabase?retryWrites=true&w=majority';
...
app.post('/gantt_pro', async (req, res) => {
  ...
  const options = { method: 'GET', headers: { Accept: 'application/json', 'X-API-Key': '6c925b688b174202bf05ace9f95cef8a' } };
  try {
...
app.post('/get_project', async (req, res) => {
  ...
  try {
    ...
    const options = {
      method: 'GET',
      headers: { Accept: 'application/json', 'X-API-Key': '6c925b688b174202bf05ace9f95cef8a' }
    };
    const { data } = await axios.get(url, options);
    res.json(data);
  } catch (error) {
...
app.post('/assign_ganttPro', async (req, res) => {
  ...
  const headers = { Accept: 'application/json', 'Content-Type': 'application/json', 'X-API-Key': '6c925b688b174202bf05ace9f95cef8a' };
  resources.forEach(i => {
