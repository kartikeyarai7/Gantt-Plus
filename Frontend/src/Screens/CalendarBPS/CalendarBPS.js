...
const CalendarBPS = () => {
  ...
  async function deleteHandler(i) {
    try {
      ...
      const { res } = await axios.post('/delete_bps', id);
  ...
  async function getDataHandler() {
    try {
      const { data } = await axios.get('/bps-schedule');
      setData(data);
    } catch (error) {
      console.log(error);
    }
  ...
  const download = (image, { name = 'Activity-Calendar-BPS', extension = 'jpg' } = {}) => {
  ...
  async function saveDataHandler() {
    ...
    try {
      ...
      const res = await axios.put('/edit_bps', updatedData);
      if (res.status === 200) {
  ...
  async function addActivityHandler() {
    if (inputCountry && inputCustomer && inputDoneBy && inputType && inputPmCount) {
      if (inputCountry.includes('-') || inputType.includes('-') || inputCountry.includes('-') || inputDoneBy.includes('-')) {
        ...
        curData.push(obj);
        try {
          const { res } = await axios.post('/add_bps-schedule', obj);
  ...
  return (
    <>
      ...
      <div className='px-3' ref={ref} id='section-to-print'>
        <h2 className='text-center px-4 pb-5 pt-2'>Activity Calendar - BPS</h2>
          ...
          <Table bordered hover>
            <thead className='text-light'>
              <tr>
                ...
                <th className='bg-dark'>Customer</th>
